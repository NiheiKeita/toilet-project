      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          {stalls.map((stall) => (
            <div
              key={stall.id}
              className={`relative p-8 rounded-3xl border-2 transition-all duration-300 ${getThemeClasses(stall.theme, stall.occupied)} ${!stall.occupied ? 'hover:transform hover:scale-105 cursor-pointer' : ''
                }`}
            ...
            >
              ...
              <div className="flex flex-col items-center">
                <div className="relative mb-6">
                  {/* Toilet Bowl */}
                  <div className={`w-32 h-40 rounded-3xl border-4 ${stall.occupied ? 'border-gray-300 bg-gray-100' : 'border-current bg-white/50'
                    } shadow-inner relative overflow-hidden`}>
                    {/* Water */}
                    <div className={`absolute bottom-2 left-2 right-2 h-8 rounded-2xl ${stall.occupied ? 'bg-gray-200' : 'bg-blue-200'
                  ...
                  {/* Flush Handle */}
                  <div className={`absolute -right-4 top-4 w-6 h-12 rounded-lg ${stall.occupied ? 'bg-gray-400' : 'bg-current opacity-60'
                    }`}></div>
                </div>
              ...
              {/* Availability Badge */}
              <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-medium ${stall.occupied
                ? 'bg-red-100 text-red-600'
                : 'bg-green-100 text-green-600'
                }`}>
                {stall.occupied ? '壊れています' : '空室'}
              </div>
            </div>
          ))}
        </div>
